...
const Carousel = ({ images, labels }) => {
  ...
  return (
    <div className="h-[20rem] md:h-[22rem] py-[1rem] md:py-[2rem] bg-black relative overflow-hidden">
      ...
      <div className="flex md:hidden transition-transform duration-1000 ease-in-out relative" style={{ transform: `translateX(-${currentIndex * 96}%)`}}>
        {images.map((image, index) => (
          <img key={index} src={image} className="rounded-lg h-[18rem] cursor-pointer absolute object-fill" style={{"width": "92%", "margin-left": `${index*96 + 4}%`}} onClick={handleImageClick}/>
        ))}
      </div>
      ...
      <button onClick={() => {handleNext(4)}} className="hidden md:block top-1/2 right-0 transition-transform duration-200 ease-in-out transform hover:scale-110 active:scale-90 rounded-full mr-3 bg-slate-700/90 text-white/90 -translate-y-1/2 px-4 py-2 focus:outline-none">
